...
ENV OPENMS_REPOSITORY https://github.com/OpenMS/OpenMS.git
...
RUN cd /usr/local/  && \
    ...
    cmake -DCMAKE_PREFIX_PATH="/usr/local/smartPeak_dependencies-build/;/usr/local/smartPeak_dependencies/;/usr/;/usr/local" -DBOOST_USE_STATIC=OFF -DHAS_XSERVER=Off ../OpenMS && \
    # #Debug
